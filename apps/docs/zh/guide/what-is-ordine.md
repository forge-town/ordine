...
代码质量自动化是成序的首个旗舰用例，作为内置插件交付。随着时间推移，领域特定能力（代码审查、安全扫描、文档生成等）将完全抽取为插件，成序本身将成为纯粹的调度核心。

## 问题

自动化工作流无处不在——代码审查、CI/CD、数据处理、内容生成——但每个领域都在重新发明自己的调度层。团队最终面对的是碎片化的工具，它们无法组合、共享上下文，也无法有效利用 AI agent。
...
该插件展示了 Ordine 的可扩展性模型：领域知识存在于插件中，调度逻辑存在于核心中。
