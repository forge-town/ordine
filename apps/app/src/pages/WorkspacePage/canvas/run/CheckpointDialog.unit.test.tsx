...
vi.mock("@refinedev/core", () => ({
  useDataProvider: () => () => ({ custom: customMock }),
}));
...
describe("CheckpointDialog", () => {
  beforeEach(() => customMock.mockReset());
  ...
  it("cancels the job on reject", async () => {
    const user = userEvent.setup();
    customMock.mockResolvedValue({ data: { cancelled: true } });
    const store = createCanvasStore({ nodes: [operationNode] });
    store.getState().applyJobSnapshot(waitingJob);
    render(<CheckpointDialog />, { wrapper: makeWrapper(store) });

    await user.click(screen.getByTestId("checkpoint-reject"));

    await waitFor(() =>
      expect(customMock).toHaveBeenCalledWith(
        expect.objectContaining({ payload: { jobId: "job-1" }, url: "jobs/cancel" }),
      ),
    );
    expect(store.getState().latestJob?.status).toBe("cancelled");
  });
});
