...
export const ProgressList = ({ items, showStatus = false }: ProgressListProps) => (
  <div className="space-y-1 border-l border-border pl-3 text-[11.5px]" data-testid="agent-progress-list">
    {items.map((item) => (
      <div className="flex items-center gap-1.5" key={item.id}>
        {showStatus ? (
          item.done ? (
            ...
            <Icon className="shrink-0 animate-spin text-muted-foreground" icon={LoaderCircle} size={11} />
          )
        ) : null}
