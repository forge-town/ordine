...
const getStatusKey = (status: JobStatus): string => {
...
const getTypeKey = (type: JobType): string => {
  const keyMap: Record<JobType, string> = {
    pipeline_run: "typePipeline",
    code_analysis: "typeCodeAnalysis",
    skill_execution: "typeSkillExecution",
    file_scan: "typeFileScan",
    custom: "typeCustom",
  };
  return keyMap[type];
};
  ...
  if (!job) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2 text-center">
        ...
        <p className="text-sm font-medium text-muted-foreground">{t("jobs.notFound")}</p>
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-background px-6">
        ...
        >
          ...
          {t(`jobs.${getStatusKey(job.status)}`)}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        {job.error && (
          <div className="rounded-xl border border-red-100 bg-red-50 p-4">
            <p className="mb-1.5 text-xs font-semibold text-red-600">{t("jobs.errorInfo")}</p>
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex items-center gap-2 border-b border-border bg-muted/50 px-4 py-2.5">
            ...
            <span className="ml-auto text-[11px] text-muted-foreground">
              {job.logs.length} {t("jobs.linesCount")}
            </span>
          </div>
          {job.logs.length === 0 ? (
