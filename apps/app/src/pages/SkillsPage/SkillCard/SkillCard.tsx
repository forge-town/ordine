...
export const SkillCard = ({ item, onCreateOperation, onDelete }: SkillCardProps) => {
  ...
  return (
    <div className="group flex min-h-[186px] flex-col rounded-lg bg-surface p-3.5 ring-1 ring-border shadow-soft transition-all hover:shadow-float hover:ring-border-strong">
      <div className="flex items-start gap-2.5">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-surface-2">
          <Icon className="text-foreground/75" icon={Wand2} size={14} />
        </span>
        <div className="min-w-0 flex-1">
          <div className="truncate font-mono text-[12.5px] font-semibold tracking-tightish">
            {item.name}
          </div>
      ...
      <div className="mt-auto flex items-center gap-1.5 pt-2.5 text-[10.5px] text-muted-foreground">
        ...
        <span className="min-w-0 truncate">powers {item.operation}</span>
        <div className="ml-auto flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
          <Button
            aria-label={`Create operation from ${item.name}`}
            size="icon-sm"
            variant="ghost"
