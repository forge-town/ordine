  ...
  for (const node of ordered) {
    ...
    if (node.type === "github-project") {
      ...
      if (ghData.sourceType === "local") {
        ...
        inputPath = localPath;
        const tree = await listDirTree(localPath);
        currentContent = `Local Folder: ${localPath}\n\nFile tree:\n${tree}`;
        await log(`@@NODE_DONE::${node.id}`);
        continue;
      }
    ...
    if (node.type === "operation") {
      ...
      if (executor.type === "script") {
        ...
        const promptResult = await runPrompt(executor, effectiveInput, llmOverride, handleChunk);
