import { z } from "zod/v4";

export const serverEnvSchema = z.object({
  DATABASE_URL: z.string().min(1),
  ORDINE_AGENT_API_TOKEN: z.string().min(32).optional(),
  ORDINE_API_PROXY_TARGET: z.url().default("http://localhost:9433"),
  ORDINE_EXECUTION_API_TARGET: z.url().optional(),
  ORDINE_EXECUTION_APP_TOKEN_FILE: z.string().min(1).optional(),
  ORDINE_EXECUTION_AGENT_TOKEN_FILE: z.string().min(1).optional(),
  ORDINE_EXECUTION_OWNER_USER_ID: z.string().min(1).optional(),
  ORDINE_EXECUTION_AUTH_MODE: z.enum(["bearer", "desktop"]).default("bearer"),
  VITE_APP_URL: z.string().default("http://localhost:9430"),
  BETTER_AUTH_SECRET: z.string(),
  GITHUB_CLIENT_ID: z.string().optional(),
  GITHUB_CLIENT_SECRET: z.string().optional(),
  GOOGLE_CLIENT_ID: z.string().optional(),
  GOOGLE_CLIENT_SECRET: z.string().optional(),
  NODE_ENV: z.enum(["development", "production", "test"]).default("development"),
  RUNTIME_SCAN_MODE: z.enum(["daemon", "local"]).default("daemon"),
  ORDINE_LOCAL_MODE: z
    .enum(["true", "false"])
    .default("false")
    .transform((v) => v === "true"),
  ORDINE_SELF_HOSTED: z
    .enum(["true", "false"])
    .default("false")
    .transform((v) => v === "true"),
});
