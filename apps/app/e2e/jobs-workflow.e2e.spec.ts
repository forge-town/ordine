...
test.describe("Job execution workflow", () => {
  test("runs a Script Operation Pipeline and reopens the persisted Job", async ({
    ...
    await page
      .getByPlaceholder("e.g. eslint src/ --fix")
      .fill(`node -e "process.stdout.write('job-${runId}')"`);
    ...
    const operationEntry = componentPanel.getByRole("button", { name: new RegExp(operationName) });
