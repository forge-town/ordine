...
const RUN_PLAN = [
  ...
  },
  {
    ...
    after: [["info", "checkpoint approved — continuing to verify"]],
  },
  {
    id: "n4",
  ...
  return (
    <div className="pointer-events-auto absolute inset-x-3 bottom-16 z-30">
      <div className="overflow-hidden rounded-2xl bg-surface shadow-win ring-1 ring-border-strong">
        <button
          onClick={onToggle}
          className="flex w-full items-center gap-2 border-b border-border/70 px-3.5 py-2 text-left"
        >
          <span className="flex h-5 w-5 items-center justify-center rounded-md bg-surface-2">
            <RIcon name="SquareTerminal" size={12} className="text-foreground/75" />
          </span>
        ...
        {open && (
          <div
            ref={ref}
            className="overflow-y-auto px-3.5 py-2.5 font-mono text-[10.5px] leading-[1.65]"
            style={{ height }}
          >
            {log.length === 0 && (
              <div className="text-muted-foreground/70">Waiting for the first step…</div>
            )}
            {log.map((l, i) => {
                ...
                <div key={i} className={`flex gap-2 ${isHead ? "mt-1.5 first:mt-0" : ""}`}>
                  ...
                  {isHead ? (
                    <span className="truncate">
                      <span className="text-foreground/45">@@NODE_START</span>{" "}
                      <span className="font-semibold text-foreground/90">{l.name}</span>
                    </span>
                  ) : l.type === "done" ? (
                    <span className="text-foreground/55">
                      @@NODE_DONE {l.name} · {l.text}
                    </span>
                  ) : (
                    <span className={s.cls} style={color ? { color } : undefined}>
                      {l.text}
                    </span>
                  )}
                </div>
      ...
      <div className="node-config-card relative w-[420px] overflow-hidden rounded-2xl bg-surface shadow-win ring-1 ring-border-strong">
        <div className="flex items-center gap-2.5 border-b border-border/70 px-4 py-3">
          <span
            ...
            style={{ background: "color-mix(in oklab, var(--c-warning) 18%, transparent)" }}
          >
            <RIcon
              name="Flag"
              size={15}
              style={{ color: "color-mix(in oklab, var(--c-warning) 72%, var(--c-fg))" }}
            />
          </span>
          <div className="flex-1">
            <div className="text-[13.5px] font-semibold tracking-tightish">
              Checkpoint · review before continuing
            </div>
            <div className="text-[10.5px] text-muted-foreground">
              Paused after <span className="font-medium text-foreground/80">{node.name}</span> · the
              run is holding
            </div>
          </div>
        </div>
          ...
          <div className="rounded-xl bg-surface-2 p-3 ring-1 ring-border">
            ...
            <div className="space-y-1 text-[11.5px] leading-relaxed text-foreground/85">
              <div className="flex items-center gap-1.5">
                <RIcon name="Check" size={11} style={{ color: "var(--c-success)" }} /> 20
                multiple-choice questions drafted
              </div>
              <div className="flex items-center gap-1.5">
                <RIcon name="Check" size={11} style={{ color: "var(--c-success)" }} /> 4 options
                each · difficulty varied
              </div>
              <div className="flex items-center gap-1.5 text-muted-foreground">
                <RIcon name="ArrowRight" size={11} /> next: adversarial verify, then Notion export
              </div>
            </div>
          </div>
          <p className="mt-2.5 text-[11.5px] leading-relaxed text-muted-foreground">
            This step is marked as a checkpoint. Approve to let the run continue, or send it back
            with a note.
          </p>
        </div>
        <div className="flex items-center gap-2 border-t border-border/70 px-4 py-3">
          <button
            onClick={onApprove}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-foreground py-2 text-[12.5px] font-medium text-primary-foreground hover:opacity-90"
          >
            <RIcon name="Play" size={13} className="fill-current" /> Approve & continue
          </button>
          <button
            onClick={onEdit}
            className="rounded-xl bg-surface px-3 py-2 text-[12.5px] ring-1 ring-border hover:bg-accent/60"
          >
            Edit step
          </button>
          <button
            onClick={onReject}
            className="rounded-xl px-3 py-2 text-[12.5px] text-muted-foreground hover:bg-accent/60"
          >
            Stop
          </button>
        </div>
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 w-[200px] rounded-2xl bg-surface p-2.5 shadow-float ring-1 ring-border-strong fade-rise">
          <div className="px-1 pb-1.5 text-[9.5px] font-semibold uppercase tracking-[0.1em] text-muted-foreground">
            Node run states · 9
          </div>
            ...
            {STATE_DEFS.map(([k, label, tone]) => (
              <div key={k} className="flex items-center gap-2 rounded-lg px-1.5 py-1 text-[11px]">
                <span className="relative inline-flex h-[9px] w-[9px] items-center justify-center">
                  {tone === "neutral" ? (
                    <span
                      className="h-[9px] w-[9px] rounded-full bg-surface"
                      style={{
                        boxShadow:
                          "inset 0 0 0 1.5px color-mix(in oklab, var(--c-muted-fg) 55%, transparent)",
                      }}
                    />
                  ) : (
                    <span
...
function VersionMenu({ version, dirty, runState, onOverwrite, onSaveAsNew }) {
  const [open, setOpen] = useStateR(false);
  const stateLabel =
    runState === "running" ? "running" : runState === "done" ? "completed" : "draft";
  const versions = [];
        ...
        {dirty ? (
          <span className="flex items-center gap-1 text-foreground/80">
            <span
              ...
              style={{ background: "color-mix(in oklab, var(--c-warning) 80%, var(--c-fg))" }}
            />{" "}
            unsaved
          </span>
        ) : (
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full z-40 mt-1.5 w-[208px] rounded-2xl bg-surface p-1.5 shadow-float ring-1 ring-border-strong fade-rise">
            ...
            >
              <RIcon name="Save" size={13} className="text-muted-foreground" />{" "}
              <span className="flex-1">Overwrite v{version}</span>
            </button>
            <button
              onClick={() => {
                onSaveAsNew();
            ...
            >
              <RIcon name="GitBranch" size={13} className="text-muted-foreground" />{" "}
              <span className="flex-1">Save as new version</span>
            ...
            <div className="max-h-[132px] overflow-y-auto">
              {versions.map((v) => (
                ...
                >
                  <RIcon
                    name={v === version ? "GitCommitHorizontal" : "GitCommit"}
                    size={13}
