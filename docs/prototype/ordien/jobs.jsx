...
const RoutineStore = {
  ...
  set(next) {
    ...
    this.subs.forEach((f) => f(this.list));
  },
  subscribe(f) {
    this.subs.add(f);
...
function weekEvents(weekStart, routines, ops) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const mk = (pipe, h, m, extra) => {
      ...
      return {
        id: "job_" + dn.toString(16) + h.toString(16) + pipe.replace(/[^A-Z]/g, "").toLowerCase(),
        pipe,
        t,
        status: past ? "completed" : "scheduled",
        step: 4,
        total: 4,
        stepName: past ? "Completed" : "Scheduled",
        agent: "Codex",
        by: "routine",
        dur: 18 + (dn % 9) + "." + (dn % 10) + "s",
        cost: "$0.0" + (6 + (dn % 4)),
    ...
    (routines || [])
      ...
      .forEach((r) => {
        const t = new Date(day);
        t.setHours(r.h, r.m, 0, 0);
        if (t.getTime() > Date.now())
          evs.push({
            id: "ghost_" + r.id + "_" + t.getTime(),
            routineId: r.id,
            pipe: r.pipe,
            t,
            status: "scheduled",
...
function Act({ icon, label, onClick, solid }) {
  return (
    ...
    >
      <Icon name={icon} size={solid ? 11 : 13} />
      {solid ? label : null}
    </button>
  );
}
...
function TableView({ jobs, onOpen, onAct }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-surface ring-1 ring-border shadow-soft">
      <div
        className={`grid ${cols} items-center gap-3 border-b border-border/70 bg-surface-2/50 px-3.5 py-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground`}
      >
        <span>Job</span>
        <span>Status</span>
        <span>Started</span>
      ...
      <div className="divide-y divide-border/60">
        {jobs.map((j) => (
          <div
            key={j.id}
            onClick={() => onOpen(j)}
            className={`group grid ${cols} cursor-pointer items-center gap-3 px-3.5 py-2.5 transition-colors hover:bg-accent/30`}
          >
            ...
            <div className="flex items-center justify-end gap-0.5">
              {j.status === "waitingForUser" ? (
                ...
                opsActions(j).map(([ic, lb]) => (
                  <Act key={lb} icon={ic} label={lb} onClick={() => onAct(lb, j)} />
                ))
              )}
            </div>
          </div>
...
function evStyle(ev) {
  ...
  if (ev.status === "waitingForUser" || ev.status === "paused")
...
function CalendarView({
  ...
  const weekStart = useMemo(
    () => new Date(monday(NOW).getTime() + weekOff * 7 * DAY_MS),
    [weekOff],
  );
  const days = useMemo(
    ...
    <div className="overflow-hidden rounded-2xl bg-surface ring-1 ring-border shadow-soft">
      ...
      <div className="flex items-center gap-2 border-b border-border/70 px-3.5 py-2.5">
        ...
        <span className="text-[10.5px] text-muted-foreground">
          {weekOff === 0 ? "this week" : weekOff > 0 ? `+${weekOff}w` : `${weekOff}w`}
        </span>
        <div className="ml-auto flex items-center gap-1">
          <span className="mr-2 hidden items-center gap-3 text-[10px] text-muted-foreground lg:flex">
            <span className="inline-flex items-center gap-1">
              <span className="h-2 w-3 rounded-sm bg-surface ring-1 ring-border" /> ran
            </span>
            <span className="inline-flex items-center gap-1">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg bg-foreground px-2.5 py-1.5 text-[11px] font-medium text-primary-foreground hover:opacity-90"
          >
      ...
      >
        ...
        {days.map((d, i) => {
          ...
          return (
            ...
            >
              <span
                className={`text-[10px] uppercase tracking-[0.08em] ${today ? "font-semibold text-foreground" : "text-muted-foreground"}`}
              >
                {d.toLocaleDateString("en-US", { weekday: "short" })}
              </span>
              <span
                className={`flex h-5 w-5 items-center justify-center rounded-full text-[11px] tabular-nums ${today ? "bg-foreground font-semibold text-primary-foreground" : "text-foreground/80"}`}
              >
                {d.getDate()}
              </span>
            </div>
      ...
      <div className="flex items-center gap-2 border-b border-border/70 bg-surface-2/40 px-3.5 py-1.5">
        <span className="w-[36px] shrink-0 text-[9px] font-semibold uppercase tracking-[0.1em] text-muted-foreground">
          live
        </span>
        {routines
          .filter((r) => r.trigger === "event")
          .map((r) => (
            <button
              key={r.id}
            ...
            >
              <Dot tone="muted" ping /> {r.pipe}{" "}
              <span className="font-mono text-[9.5px] text-muted-foreground">on {r.event}</span>
            </button>
          ))}
        {!routines.some((r) => r.trigger === "event") && (
          <span className="text-[10px] text-muted-foreground">no event-triggered routines</span>
        )}
      </div>
      {/* week grid */}
      <div
        className="relative grid"
        style={{ gridTemplateColumns: "48px repeat(7, 1fr)", height: gridH }}
      >
        {/* hour labels + lines */}
        <div className="relative">
          {Array.from({ length: CAL_END - CAL_START }, (_, i) => (
            ...
            >
              {i === 0 ? "" : String(CAL_START + i).padStart(2, "0") + ":00"}
            </div>
          ))}
        </div>
        {days.map((d, di) => {
          const today = d.toDateString() === NOW.toDateString();
          const dayEvs = events
            .filter((e) => e.t && e.t.toDateString() === d.toDateString())
            .map((ev) => ({
              ev,
              top: Math.max(0, (ev.t.getHours() + ev.t.getMinutes() / 60 - CAL_START) * HOUR_H),
            }))
          ...
          dayEvs.forEach((p, i) => {
            ...
            p.cols = cluster.length;
            p.col = cluster.indexOf(p);
          });
          return (
            <div
              key={di}
              className={`relative border-l border-border/50 ${today ? "bg-accent/25" : ""}`}
            >
              {Array.from({ length: CAL_END - CAL_START }, (_, i) => (
                <div
                  key={i}
              ...
              {dayEvs.map(({ ev, top, col, cols }) => {
                ...
                return (
                  ...
                  >
                    {ev.status === "running" && (
                      <span className="h-1 w-1 shrink-0 animate-pulse rounded-full bg-primary-foreground" />
                    )}
                    {ev.status === "failed" && (
                      <span
                    ...
                    {cols === 1 && (
                      <span
                        className={`ml-auto shrink-0 font-mono text-[8.5px] ${ev.status === "running" ? "opacity-70" : "text-muted-foreground"}`}
                      >
                        {hhmm(ev.t)}
                      </span>
                    )}
              ...
              {today && nowTop > 0 && nowTop < gridH && (
                ...
                >
                  <div
                    ...
                    style={{
                      background: "color-mix(in oklab, var(--c-destructive) 75%, transparent)",
                    }}
                  />
                  <div
                    className="absolute -left-0.5 -top-[2.5px] h-[5px] w-[5px] rounded-full"
                    style={{ background: "var(--c-destructive)" }}
                  />
                </div>
              )}
...
function JobsPage({ onOpen, notify }) {
  ...
  const act = (lb, j) => {
    const patch =
      lb === "Pause"
        ? { status: "paused", dur: j.dur }
        : lb === "Resume"
          ? { status: "running" }
          : lb === "Stop" || lb === "Cancel"
            ? { status: "cancelled" }
            : lb === "Rerun"
              ? { status: "running", t: new Date(), dur: "0.2s", cost: "$0.00", tok: "—" }
              : null;
  ...
  const saveRoutine = (cfg) => {
    setRoutines((rs) => {
      let next =
        sched && sched.mode === "edit" ? rs.filter((r) => r.id !== sched.routine.id) : rs.slice();
  ...
  const editRoutine = (id) => {
    ...
    if (r) setSched({ mode: "edit", routine: r });
  };
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <PageHeader
        ...
        actions={
          <>
            <Btn variant="ghost" icon="Clock" onClick={() => setSched({ mode: "new" })}>
              New Routine
            </Btn>
      ...
      <div className="flex items-center gap-3 px-7 pb-3.5">
        <Seg
          ...
          onChange={setView}
          options={[
            { v: "list", label: "List", icon: "Rows3" },
            { v: "calendar", label: "Calendar", icon: "CalendarDays" },
          ]}
        />
        {view === "list" && (
          <span className="text-[11px] text-muted-foreground">
            {counts.running} running · {counts.queued} queued · {counts.waiting} waiting on you ·{" "}
            {counts.failed} failed today
          </span>
        )}
        {view === "list" && (
          <div className="ml-auto w-56">
            <JSearch value={q} onChange={setQ} />
          </div>
        )}
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-7 pb-8 pt-0.5">
        {view === "list" ? (
          ...
          <CalendarView
            ...
            ops={ops}
            onOpenJob={setDetail}
            onNewRoutine={() => setSched({ mode: "new" })}
        ...
        <ScheduleEditor
          pipe={
            sched.mode === "new"
              ? { pickable: true }
              : {
                  name: sched.routine.pipe,
                  schedule: sched.routine.trigger === "event" ? "On event" : "Daily",
                }
          }
          initial={
            sched.mode === "edit"
              ? {
                  trigger: sched.routine.trigger,
                  cron: sched.routine.cron,
                  event: sched.routine.event,
                }
              : undefined
          }
