  ...
  return useMemo(() => {
    ...
    (S.pipes || []).forEach((p) =>
      out.push({
        ...
        sub: p.desc,
        route: "workspace",
        icon: p.draft ? "MessageSquare" : "Workflow",
      }),
    );
    Object.keys(S.comps || {}).forEach((cat) =>
      (S.comps[cat].items || []).forEach((it) =>
        out.push({
          ...
          sub: it.meta || cat,
          route: "components",
          icon: S.comps[cat].icon,
        }),
      ),
    );
    (window.PIPE_NODES || []).forEach((n) =>
      out.push({
        type: "Node",
        label: n.title,
        sub: n.kind + " · in Textbook → Quiz",
        route: "workspace",
        icon: n.icon,
      }),
    );
    ...
    (S.skills || []).forEach((s) =>
      out.push({ type: "Skill", label: s.name, sub: s.desc, route: "skills", icon: "Sparkles" }),
    );
    (S.conns || []).forEach((c) =>
      out.push({
        type: "Connector",
        label: c.name,
        sub: c.method + " · " + c.scopes,
        route: "connectors",
        icon: "Plug",
      }),
    );
    (S.agents || []).forEach((a) =>
      out.push({ type: "Agent", label: a.name, sub: a.models, route: "agents", icon: "Cpu" }),
    );
    return out;
  }, []);
}
...
function GlobalSearchResults({ q, onPick }) {
  ...
  const hits = ql
    ? corpus
        .filter(
          (c) => c.label.toLowerCase().includes(ql) || (c.sub || "").toLowerCase().includes(ql),
        )
        .slice(0, 24)
    : [];
  const groups = {};
        ...
        order
          ...
          .map((g) => (
            <div key={g} className="mt-2.5 first:mt-0">
              <div className="px-2.5 py-1 text-[9.5px] font-semibold uppercase tracking-[0.11em] text-muted-foreground">
                {g}s<span className="ml-1 text-muted-foreground/60">{groups[g].length}</span>
              </div>
              <ul className="space-y-0.5">
                {groups[g].map((h, i) => (
                  <li key={g + i}>
                    <button
                      onClick={() => onPick(h)}
                      className="group flex w-full items-center gap-2.5 rounded-xl px-2.5 py-1.5 text-left transition-colors hover:bg-accent/60"
                    >
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-surface-2">
                        <SIcon name={h.icon} size={12} className="text-foreground/70" />
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-[12px] font-medium tracking-tightish">
                          {h.label}
                        </span>
...
function Sidebar({
  ...
  const badgeCounts = useMemo(() => {
    const S = window.SEARCH || {};
    const compTotal = Object.keys(S.comps || {}).reduce(
      (n, c) => n + (S.comps[c].items || []).length,
      0,
    );
  ...
  const pick = (hit) => {
    setQ("");
    onNav(hit.route);
    notify(`Opened ${hit.type.toLowerCase()} · ${hit.label}`);
  };
        ...
        <div className="px-3 pt-3">
          <div className="flex items-center gap-1.5">
            <button
              ...
              title="Back"
              className="flex flex-1 items-center gap-2 rounded-xl px-2 py-1.5 text-left text-[13px] font-medium text-foreground/80 transition-colors hover:bg-accent/60 hover:text-foreground"
            >
            ...
            <button
              ...
              className="shrink-0 rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-accent/60 hover:text-foreground"
            >
              <SIcon name="PanelLeft" size={15} />
            </button>
          </div>
                  ...
                  {projects.map((p) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="truncate text-[10px] text-muted-foreground">{p.sub}</div>
                      </div>
                      {p.id === proj && (
                        <SIcon name="Check" size={13} className="text-foreground" />
                      )}
                    </button>
                  ))}
          ...
          <ul className="space-y-0.5">
            {(window.SETTINGS_GROUPS || []).map((g) => {
              ...
              return (
                <li key={g.key}>
                  <button
                    onClick={() => setSettingsAnchor(g.key)}
            ...
            {userMenu.map(([ic, label, act]) => (
              ...
              >
                <SIcon
                  name={ic}
                  size={13}
                  className={label === "Sign out" ? "" : "text-muted-foreground"}
                />{" "}
                {label}
              </button>
            ))}
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1 truncate text-[10px] text-muted-foreground">
                <span className="inline-block h-1.5 w-1.5 rounded-full bg-success" /> Local mode · 3
                agents
              </div>
            </div>
          </button>
...
function NotifBell({ notifs, onToggle }) {
  ...
  return (
    <button
      onClick={onToggle}
      title="Notifications"
      className="pointer-events-auto relative rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-accent/60 hover:text-foreground"
    >
      <SIcon name="Bell" size={15} />
      {unread > 0 && (
        <span className="absolute -right-0.5 -top-0.5 flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-foreground px-1 text-[8.5px] font-bold text-primary-foreground">
          {unread}
        </span>
      )}
    </button>
...
const NOTIF_ICON = {
  success: ["CircleCheck", "var(--c-success)"],
  error: ["CircleAlert", "var(--c-destructive)"],
  warn: ["TriangleAlert", "color-mix(in oklab, var(--c-warning) 80%, var(--c-fg))"],
  info: ["Info", "var(--c-muted-fg)"],
};
function NotifCenter({ notifs, onClose, onClear, onMarkRead, onNav }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute right-3 top-10 z-50 w-[320px] overflow-hidden rounded-2xl bg-surface shadow-win ring-1 ring-border-strong fade-rise"
    >
      ...
      <div className="max-h-[360px] overflow-y-auto py-1">
        {notifs.length === 0 ? (
          <div className="grid place-items-center px-4 py-10 text-center">
            <SIcon name="BellOff" size={20} className="text-muted-foreground/50" />
            <div className="mt-2 text-[12px] font-medium">All clear</div>
            <div className="mt-0.5 text-[11px] text-muted-foreground">
              Run, self-heal & connector events show here.
            </div>
          </div>
        ) : (
          notifs.map((n) => {
            ...
            return (
              ...
              >
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center">
                  <SIcon name={ic} size={14} style={{ color }} />
                </span>
                <span className="min-w-0 flex-1">
...
function MacWindow({ children, rightSlot }) {
  return (
    <div
      className="flex flex-col overflow-hidden rounded-[14px] bg-background shadow-win ring-1 ring-black/10"
      style={{ width: 1380, height: 858 }}
    >
...
function ResizeHandle({ side, onDelta, onCommit, onCollapse, line = true }) {
  ...
  const down = (e) => {
    ...
    const move = (ev) => {
      ...
      onDelta(dx, (collapsed) => {
        start.current.collapsed = collapsed;
      });
    };
    const up = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute inset-y-6 left-1/2 w-1 -translate-x-1/2 rounded-full opacity-0 transition-opacity group-hover:opacity-100"
        style={{ background: "color-mix(in oklab, var(--c-fg) 24%, transparent)" }}
      />
    </div>
  );
}
...
function ScaledStage({ children }) {
  ...
  useEffect(() => {
    const fit = () => {
      const s = Math.min(1, (window.innerWidth - 40) / 1380, (window.innerHeight - 40) / 858);
      setScale(s);
      window.__ordineScale = s;
    };
    fit();
  ...
  return (
    <div className="absolute inset-0 z-[70] grid place-items-center bg-background">
      <div className="flex max-w-[300px] flex-col items-center text-center fade-rise">
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-foreground text-primary-foreground shadow-float">
          <SIcon name="Lock" size={20} />
        </div>
        <div className="mt-4 text-[15px] font-semibold tracking-tightish">
          Signed out of local mode
        </div>
        <p className="mt-1 text-[12px] leading-relaxed text-muted-foreground">
          Your pipelines and conversations stay on this machine. Sign back in to pick up where you
          left off.
        </p>
        <button
          onClick={onBack}
          className="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-foreground px-3.5 py-2 text-[12.5px] font-medium text-primary-foreground hover:opacity-90"
        >
...
const SEED_NOTIFS = [
  ...
  {
    kind: "error",
    msg: "Notion connector needs a token to finish export",
    t: 12,
    route: "connectors",
  },
  ...
  {
    kind: "info",
    msg: "Distilled “Textbook → Notion Quiz” into a Pipeline Skill",
    t: 70,
    route: "components",
  },
];
...
function App() {
  ...
  const [projects, setProjects] = useState([
    {
      ...
      desc: "Personal automation studio — textbook pipelines, research briefs, changelogs.",
    },
    {
      id: "p2",
      name: "Acme Research",
      sub: "Team · 4 members",
      desc: "Shared research workspace for the Acme growth team.",
    },
    {
      id: "p3",
      name: "Side Projects",
      sub: "Personal workspace",
      desc: "Weekend experiments and one-off runs.",
    },
  ]);
  ...
  const [theme, setTheme] = useState(() => {
    try {
      return localStorage.getItem("ordine.theme") || "light";
    } catch (e) {
      return "light";
    }
  });
  ...
  const [openPipe, setOpenPipe] = useState({
    id: "textbook-quiz",
    name: "Textbook → Quiz Pipeline",
    fresh: false,
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("ordine.theme", theme);
    } catch (e) {}
    document.documentElement.setAttribute("data-theme", theme);
  }, [theme]);

  const Pages = window.Pages || {};
  const openSettings = (anchor) => {
    if (route !== "settings") setPrevRoute(route);
    setSettingsAnchor(anchor || "general");
    setRoute("settings");
  };
  const newProject = () => {
    const id = "p" + Date.now();
    setProjects((ps) => [
      ...ps,
      { id, name: "Untitled project", sub: "Personal workspace", desc: "" },
    ]);
    setProj(id);
    notify("Created “Untitled project”");
  };
  const newPipeline = () => {
    setOpenPipe({ id: "pipe" + Date.now(), name: "Untitled pipeline", fresh: true });
    setRoute("workspace");
    notify("New pipeline — describe a goal in the Agent Bar");
  };
  const openPipeline = (name) => {
    setOpenPipe({
      id: "pipe-" + (name || "demo").toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      name: name || "Pipeline",
      fresh: false,
    });
    setRoute("workspace");
  ...
  if (route === "workspace")
    main = (
      <window.Workspace
        key={openPipe.id}
        pipe={openPipe}
        onRename={(name) => setOpenPipe((p) => ({ ...p, name }))}
        agentOpen={agentOpen}
    ...
    main = <div className="grid flex-1 place-items-center text-muted-foreground">Coming soon</div>;

  return (
    <ScaledStage>
      <div className="relative">
        <MacWindow
          rightSlot={<NotifBell notifs={notifs} onToggle={() => setNotifOpen((v) => !v)} />}
        >
          {sidebarOpen && (
            <div
              className="flex h-full shrink-0 panel-in"
              style={{ width: leftW }}
              onPointerDownCapture={() => {
                leftRef.current = leftW;
              }}
            >
              <div className="h-full min-w-0 flex-1 overflow-hidden">
                <Sidebar
                  ...
                  onExitSettings={() => setRoute(prevRoute)}
                />
              </div>
            </div>
          )}
        ...
        {notifOpen && (
          <NotifCenter
            ...
            onMarkRead={() => setNotifs((ns) => ns.map((n) => ({ ...n, read: true })))}
            onNav={setRoute}
          />
        )}
