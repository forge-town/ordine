...
**过渡期诚实退化提醒**：N22-03 已禁掉"手设 connected"，但 UI（N22-04 前）的 Connect 按钮仍走旧 `update{status:connected}` → 现在会被守卫回落 needs_setup（不再假绿）。这是预期的诚实退化，待 N22-04 改调真 connect 后恢复"绿=真握手"。
...
- [ ] 真机：filesystem MCP 全链路（Connect 列工具→Run 见 mcp\_\_ 调用→坏配置报 error）证据存 pr-assets
