...
- 主线程由 Sol 保留需求、决策、风险、集成审查和最终结果；搜索输出、测试日志和中间探索尽量留在 Terra 子代理线程。
- 读密集且互不依赖的探索、检索、测试和日志分析可以并行；会修改相同文件或同一代码区域的任务必须串行。
- 未使用独立 worktree 时，不允许多个子代理同时修改同一代码区域。保持改动最小化，禁止顺手重构无关代码。
- Sol 只能向 Terra 委派目标、允许范围、禁止范围、背景、完成标准、验证、回滚和返回格式均明确的任务。
- Terra 遇到歧义、风险或越界需求时停止扩展并返回 Sol，不得自行改变架构、扩大范围或增加生产依赖。
- Terra 报告完成不代表任务通过；Sol 必须检查实际 diff、命令退出状态和测试证据后决定接受、返工、补测或回滚。
- 不得以“测试命令已启动”代替“测试已通过”；关键判断必须提供文件、符号、命令或测试证据，不能只报告结论。
- Sol High 仅用于跨系统架构、不可逆迁移、协议、安全、权限、并发、一致性、重大长期权衡或高故障影响的决策；普通功能、简单 Bug、常规测试、搜索和文档不得升级。
- `$sol-terra` 是正式工作流入口；`$sol-luna` 仅作为兼容别名并同样路由到 Terra，直到 Codex 支持 Sol v2 原生委派 Luna v1。
