...
export const PROPOSE_SYSTEM_PROMPT = [
  ...
  "{",
  '  "reply": "<substantive conversational response to the user>",',
  ...
  "- Make 'reply' useful on its own: usually 2-4 short paragraphs explaining what you changed, why the graph uses this serial/parallel dependency structure, any important assumption or open point, and what the user can do next. Do not answer with only 'done', 'drafted', or one summary sentence.",
  ...
  "- Each operation receives only its immediate parent output. For newOperations that organize, revise, validate, correct, format, or export an artifact, the prompt MUST require the complete resulting artifact in the response; it must forbid summary-only output, placeholders, and references to omitted prior content.",
  "- Avoid chains of agents that repeatedly rewrite the same large deliverable when one generation Operation with loopEnabled quality review can produce the final artifact safely.",
...
/**
 * Server-resolved run context: the job plus its most recent traces, which
...
export type BuildProposeUserPromptInput = {
  ...
  /**
   * Stage-one analysis result; when absent the prompt neither injects nor
   * claims an analysis happened.
   */
...
/**
 * Unresolved canvas anchor notes: the note content already lives in the
 * conversation history; this block indexes their locations and counts.
 */
...
/*
 * Attachment content feeds the reverse-engineering analysis. Text attachments
 * render their full content (already truncated by the frontend to the
 * 32k/128k limits; truncated again here as a backstop); binary attachments
 * expose only the filename and say so explicitly.
 */
