...
import { render, canvasTestDataProvider } from "../../test/test-wrapper";
import type { PipelineAgentSessionsClient } from "../../lib/pipelineAgentSessionsClient";
import { PipelineCreationWorkspace } from "./PipelineCreationWorkspace";

const { mockCreatePipeline, mockCustomRequest, mockNavigate } = vi.hoisted(() => ({
...
describe("PipelineCreationWorkspace", () => {
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    vi.spyOn(canvasTestDataProvider, "create").mockImplementation((args) =>
      mockCreatePipeline(args),
    );
    canvasTestDataProvider.custom = (args) => mockCustomRequest(args);
    ...
    render(
      <StrictMode>
        <PipelineCreationWorkspace
          active
          client={client as unknown as PipelineAgentSessionsClient}
          presentation="home"
        />
      </StrictMode>,
    );
