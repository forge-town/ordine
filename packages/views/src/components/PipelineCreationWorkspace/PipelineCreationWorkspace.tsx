...
import {
  createMaterializeGeneratedPipeline,
  type MaterializeGeneratedPipeline,
} from "../../lib/materializeGeneratedPipeline";
...
import {
  ...
  type PipelineAgentSessionsClient,
...
export const PipelineCreationWorkspace = ({
  active,
  client: clientProp,
  materializePipeline: materializePipelineProp,
  presentation = "dialog",
  runtimeConfigured,
  runtimeId,
  runtimeLabel,
  ...
  const client = useMemo(
    () => clientProp ?? createPipelineAgentSessionsClient(platform),
    [clientProp, platform],
  );
  const materializePipeline = useMemo(
    () =>
      materializePipelineProp ??
      createMaterializeGeneratedPipeline({ client, dataProvider: getDataProvider() }),
    [client, getDataProvider, materializePipelineProp],
  );
  ...
  const handleSend = async () => {
    ...
    if (isHome && !sessionIdRef.current && messages.length === 0 && attachments.length === 0) {
      ...
      const createResult = await ResultAsync.fromPromise(
        getDataProvider().create<PipelineData>({
      ...
      setInputValue("");
      void navigate({ to: "/canvas", search: { id: createResult.value.data.id } });
  ...
  const handleOpenInCanvas = () => {
    ...
    handleClose?.();
    void navigate({ to: "/canvas", search: { id: createdPipelineId } });
  };
