...
/**
 * 导出 Operation 所需的客户端相关依赖，由调用方（页面组件）注入：
 * - `getOne`：Refine data provider 的 getOne，经 useDataProvider() 获取，保持客户端无关。
 * - `downloadBlob`：客户端特有的文件保存能力，经 PlatformProvider 注入（Web 走浏览器下载）。
 */
export interface ExportOperationDeps {
  getOne: NonNullable<DataProvider["getOne"]>;
...
export const exportOperation = async (op: Operation, deps: ExportOperationDeps) => {
  const { getOne, downloadBlob } = deps;
  ...
  downloadBlob(blob, `${slug}.operation.zip`);
};
