...
import { ResourceName } from "@repo/views/constants";
import { getCanvasDataProvider } from "@repo/views/lib/canvasDataProvider";
import { toastStore } from "@repo/views/store/toastStore";
...
export const createActionsSlice = (
  ...
  handleCreateSkillOperationNode: async (skill, screenPosition) => {
    const result = await ResultAsync.fromPromise(
      (async () => {
        const listResult = await getCanvasDataProvider().getList!<Operation>({
        ...
        const createResult = await getCanvasDataProvider().create!<Operation>({
          resource: ResourceName.operations,
