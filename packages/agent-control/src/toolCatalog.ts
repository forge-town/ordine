...
export const AGENT_CONTROL_TOOLS = [
  ...
  defineTool({
    name: "ordine.prepare_pipeline_run",
    title: "Prepare Pipeline run",
    description:
      "Prepare an immutable Pipeline execution request. Omit inputs to use saved Prompt values. Overrides use Pipeline input ids input-<prompt-node-id>, not Operation port ids, with arrays of typed values. Awaiting approval is not a started Job; the user reviews it in ORDINE. Never resubmit an unknown request.",
    inputSchema: RunPipelineInputSchema,
    risk: "execute",
    audiences: WRITE_AUDIENCES,
    requiredScopes: ["execute"],
    idempotency: "call-id-required",
    redactedInputPaths: ["inputs"],
    executionPreflight: "pipeline-capability-closure",
  }),
  defineTool({
    name: "ordine.prepare_operation_run",
    title: "Prepare Operation run",
    description:
      "Prepare one Operation for execution using the same request and human approval flow as a Pipeline.",
  ...
  defineTool({
    name: "ordine.prepare_routine_run",
    title: "Prepare Routine run",
    description:
      "Prepare a Routine's Pipeline for execution; human approval, when required, is handled by ORDINE without another tool submission.",
