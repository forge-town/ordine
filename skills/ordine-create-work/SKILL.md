...
```bash
# 列出所有
curl -s http://localhost:9431/api/works | python3 -m json.tool

# 按项目过滤
curl -s "http://localhost:9431/api/works?projectId=proj_001" | python3 -m json.tool

# 查看单个
curl -s http://localhost:9431/api/works/work_xxx | python3 -m json.tool

# 创建
curl -X POST http://localhost:9431/api/works \
...
# 更新状态
curl -X PATCH http://localhost:9431/api/works/work_quality_review_20250115 \
  -H "Content-Type: application/json" \
  -d '{ "status": "completed" }'

# 删除
curl -X DELETE http://localhost:9431/api/works/work_quality_review_20250115
```
...
```bash
curl -s "http://localhost:9431/api/jobs?workId=work_quality_review_20250115" | python3 -m json.tool
```
